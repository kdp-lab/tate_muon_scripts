...
#fnames = glob.glob("/local/d1/lrozanov/mucoll-tutorial-2023/reco_bib_test/1000_1_reco_bib.slcio") 



fnames = glob.glob("/local/d1/mu+mu-/reco_v3/100_150_0_notimingrequirement/4000_10_reco.slcio") 
...
for f in fnames:
    ...
    for ievt,event in enumerate(reader): 
        ...
        #             # Get layer
            ...
            #     print("Error accessing {coll_name}: {e}") 
        ...
        for mcp in mcpCollection:
            ...
            for daughter in mcp.getDaughters():
                if abs(daughter.getPDG()) in stau_ids:
                    stau_daughter = True
                    break
            ...
            mcp_stau_mom = mcp.getMomentum()
            mcp_stau_mass = mcp.getMass()
            mcp_stau_tlv = ROOT.TLorentzVector()
            mcp_stau_tlv.SetPxPyPzE(mcp_stau_mom[0], mcp_stau_mom[1], mcp_stau_mom[2], mcp.getEnergy())
            mcp_stau_b = mcp_stau_tlv.Beta()

            imcp_stau_p.append(mcp_stau_tlv.Mag())
            imcp_stau_px.append(mcp_stau_mom[0])
            imcp_stau_py.append(mcp_stau_mom[1])
            imcp_stau_pz.append(mcp_stau_mom[2])
            imcp_stau_E.append(mcp.getEnergy())
            imcp_stau_m.append(mcp_stau_mass)
            imcp_stau_beta.append(mcp_stau_b)
            imcp_stau_pt.append(mcp_stau_tlv.Perp())
            imcp_stau_eta.append(mcp_stau_tlv.Eta())
            imcp_stau_phi.append(mcp_stau_tlv.Phi())
            imcp_stau_theta.append(mcp_stau_tlv.Theta())
        ...
        for st in stau_tracks:
            ...
            ptres = (mcp_stau_tlv.Perp() - pt) / mcp_stau_tlv.Perp() 
        ...
        if n_mcp_stau > 0:
            ...
            mcp_stau_beta.append(imcp_stau_beta)
            mcp_stau_p.append(imcp_stau_p)
            mcp_stau_px.append(imcp_stau_px)
            mcp_stau_py.append(imcp_stau_py)
            mcp_stau_pz.append(imcp_stau_pz)
            mcp_stau_E.append(imcp_stau_E)
            mcp_stau_m.append(imcp_stau_m)
...
#LCRELATION
    #stau pdgid matching
data_list["mcp_stau_p"] = mcp_stau_p
data_list["mcp_stau_px"] = mcp_stau_px
data_list["mcp_stau_py"] = mcp_stau_py
data_list["mcp_stau_pz"] = mcp_stau_pz
data_list["mcp_stau_E"] = mcp_stau_E
data_list["mcp_stau_m"] = mcp_stau_m
data_list["mcp_stau_pt"] = mcp_stau_pt
data_list["mcp_stau_beta"] = mcp_stau_beta
...
output_json = "/home/tateflicker/newest_files/4000_10_fullyextended.json"
#output_json = "/home/tateflicker/newest_files/1000_1_reco_bib_test.json"
